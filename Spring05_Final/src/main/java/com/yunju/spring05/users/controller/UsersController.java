...
public class UsersController {
	...
	public String loginform(HttpServletRequest request) {
		...
		if(url==null) {//만일 없으면
			//로그인 성공후에 index페이지로 보낼 수 있도록 구성
			url=request.getContextPath()+"/home.do";
		}
		//아이디, 비밀번호가 쿠키에 저장되었는지 확인해서 저장 되었으면 폼에 출력
		Cookie[] cookies=request.getCookies();
		//저장된 아이디와 비밀번호를 담을 변수를 선언하고 초기값으로 빈 문자열 대입
		String savedId="";
		String savedPwd="";
		if(cookies!=null) {
	...
	public ModelAndView authInfo(HttpServletRequest request,
		...
		//UsersService객체를 이용해서 개인정보를 ModelAndView 객체에 담기
