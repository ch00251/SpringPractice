...
import com.yunju.spring05.cafe.dao.CafeCommentDao;
...
public class CafeServiceImpl implements CafeService{
	...
	public void getList(HttpServletRequest request) {
		/*
		 * request에 검색 keyword가 전달 될수도 있고 안될수도 있다.
		 * 1. 전달 안되는 경우 : home에서 목록보기를 누른경우
		 * 2. 전달 되는 경우 : list.do에서 검색어를 입력 후 form 전송한 경우
		 * 3. 전달 되는 경우 : 이미 검색을 한 상태에서 페이징처리 링크를 누른경우
		 */
		//검색과 관련된 파라미터를 읽어와 본다.
		String keyword=request.getParameter("keyword");
		String condition=request.getParameter("condition");
		
		//CafeDto 객체 생성(select 할 때 필요한 정보를 담기 위해)
	...
	public void getDetail(HttpServletRequest request) {
		...
		//CafeDto 객체 생성(select할때 필요한 정보를 담기 위해)
		CafeDto dto=new CafeDto();
